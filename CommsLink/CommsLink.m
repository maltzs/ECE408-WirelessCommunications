...
% ECE300.
% Task 1: BPSK, 4QAM and 16QAM on no ISI channel for all SNR values (Part 1)
% Task 2: BPSK on moderate and severe ISI channels for 12 dB SNR with equalization (Part 1)
% Task 3: BPSK on moderate and severe ISI channels for 12 dB SNR with equalization and coding (Part 2)
clear; close all; clc;
...
SNR_Vec = {0:2:16, 12, 12}; % Different SNRs used based on task
...
for task = 1:3
    SNR_Vec_Task = SNR_Vec{task};
    lenSNR = length(SNR_Vec_Task);
    M = Marys{task};
    ...
    for p = 1:length(M)
        for q = 1:length(chanTasks{task})
            ...
            for i = 1:numIter
                ...
                for j = 1:lenSNR % one iteration of the simulation at each SNR Value
                    ...
                    txNoisy = awgn(txChan,SNR_Vec_Task(j),'measured'); % Add AWGN
        ...
        if task == 1
            ...
            if M(p) == 2
                berTheory = berawgn(SNR_Vec_Task,'psk',2,'nondiff');
            else
                berTheory = berawgn(SNR_Vec_Task-10*log10(log2(M(p))),'qam',M(p)); % berawgn accepts EbNo, not SNR
            end
            hold on
            semilogy(SNR_Vec_Task,berTheory,'r');
            xlabel("SNR (dB)");
            ylabel("BER");
            legend('BER', 'Theoretical BER', 'location', 'southwest');
        else
            tableStrings = ["Equalization", "Equalization & Coding"];
            table(ber(1,1,1),ber(1,1,2),br(p), ...
                'VariableNames',["Moderate ISI BER", "Severe ISI BER","Bit Rate"], ...
                'RowNames',tableStrings(task-1))
        end
    end              % End M-ary iteration

    % Summary of bit rates vs. M-ary number for task 1
    if task == 1
        table(M,br,'VariableNames',["M", "Bit Rate"])
    end
